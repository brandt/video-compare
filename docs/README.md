# Docs

This directory contains documentation for this project.

Directories:

- This directory: General project docs.
- [design/](design/): Docs related to design decisions and rationale.
- [planning/](planning/): Docs related to WIP and future plans.
- [archive/](archive/): Docs related to past decisions that have been superseded but are kept for posterity.
- [user/](user/): Additional user-facing docs.

## General

- [Architecture](architecture.md)
- [Project structure](project-structure.md)

## Design

- [Auto-alignment](design/auto-align.md)
- [Playback buffer](design/buffer.md)
- [Input testing](design/input-testing.md)
- [Input socket](design/input-socket.md)
- [Keyboard](design/keyboard.md)
- [libplacebo integration](design/libplacebo-integration.md)

## User

- [Keyboard and mouse controls](user/keyboard-and-mouse-controls.md)
- [Windows](user/Windows.md)
