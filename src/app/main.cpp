...
#include <stdexcept>
#include <vector>
#include "app/argagg.h"
#include "display/controls.h"
#include "app/runtime_notes.h"
  ...
  try {
    argagg::parser argparser{
        {{"help", {"-h", "--help"}, "print help and exit", 0},
         ...
         {"hwaccel", {"--hwaccel"}, "FFmpeg video hardware acceleration for both sides, specified as [type][:device?[:options?]] (e.g. 'videotoolbox' or 'vaapi:/dev/dri/renderD128'); 'auto' (default), 'none'", 1},
         {"left-hwaccel", {"--left-hwaccel"}, "left FFmpeg video hardware acceleration, specified as [type][:device?[:options?]] (e.g. 'cuda', 'cuda:1', 'vulkan', 'auto', 'none')", 1},
      ...
      if (args["time-shift"]) {
        ...
        try {
          ...
          std::cout << string_sprintf(
                         "Timeshift config: multiplier=%d/%d (x%.6f), offset=%ld ms",
                         time_shift_config.multiplier.num,
                         time_shift_config.multiplier.den,
                         multiplier_value,
                         time_shift_config.offset_ms)
                    << std::endl;
        } catch (const std::logic_error& e) {
